...
/* ===== VERSION 14.4 DATA — RFP Shredder response library + demo sample (added 7/20/2026) ===== */
TOOL_DATA.rfpAnswerBank = [
  {key:"capacity", label:"Kitchen capacity & delivery network", keywords:["kitchen","capacity","production","logistics","delivery","distribution","fulfill","volume","scale"],
   block:"CookUnity operates eight production kitchens with approximately 97% continental U.S. delivery coverage and 150+ working chefs. Delivery feasibility is verified at the zip-code level before any service commitment — coverage claims in this response are checkable, not aspirational. [COMPANY FACT NEEDED: current kitchen count/production capacity for the proposed service area, from Ops.]"},
  {key:"experience", label:"Relevant experience & past performance", keywords:["experience","past performance","qualification","history","similar","previous","track record","demonstrated"],
   block:"CookUnity delivers live Medicaid medically tailored meal programs today: with CABS in Brooklyn under New York's 1115 Social Care Network mechanism (500+ members), and with Anthem in Sacramento under CalAIM Community Supports. Additional healthcare delivery includes the EmblemHealth Ornish program. [COMPANY FACT NEEDED: contract references with permission, program metrics cleared for external use.]"},
  {key:"staffing", label:"Staffing plan & key personnel", keywords:["staffing","personnel","key staff","team","qualifications of","resumes","organizational chart","fte"],
   block:"Program delivery is led by a Registered Dietitian-governed clinical function with dedicated account management. [COMPANY FACT NEEDED: named key personnel, resumes, org chart, and clinical-oversight structure — from the healthcare team; never submit placeholder names.]"},
  {key:"implementation", label:"Implementation / work plan / timeline", keywords:["implementation","work plan","timeline","transition","launch","ramp","milestones","project plan","start date"],
   block:"Implementation follows a phased plan: readiness and data-exchange setup, eligibility file intake and member outreach, first-delivery window, then steady-state operations with quarterly reporting. [COMPANY FACT NEEDED: ops-verified timeline commitments — never commit dates ops has not signed.]"},
  {key:"evidence", label:"Evidence & outcomes", keywords:["outcome","evidence","effectiveness","clinical result","impact","health improvement","readmission","utilization"],
   block:"Evidence hierarchy, labeled by source type: peer-reviewed — Berkowitz et al. (JAMA Intern Med 2019): $753/member/month lower spending (16.4%) in a matched MTM cohort; Berkowitz et al. (Health Affairs 2018): ~$220/member/month net savings among dual-eligibles; actuarial — North Carolina HOP summative evaluation: $164 PMPM; peer-reviewed (in-state MA) — Nature Medicine 2025 MassHealth study. Program-generated evidence design: agreed baseline, comparison method and attribution defined BEFORE launch. No savings guarantee is expressed or implied — payer claims data decides results."},
  {key:"reporting", label:"Reporting, data & quality measures", keywords:["report","data","dashboard","metrics","quality measure","hedis","performance measure","deliverable"],
   block:"Quarterly account-health reporting: activation vs. eligible population, reorder (adherence) rate, member satisfaction, screened-to-intervention documentation within 30 days (HEDIS SNS-E shape), and non-food referral routing with disposition. Reporting is de-identified by default; member-level exchange occurs only inside BAA-covered infrastructure. [COMPANY FACT NEEDED: data-exchange formats and file specs from the data team.]"},
  {key:"privacy", label:"HIPAA / privacy / security", keywords:["hipaa","privacy","security","phi","confidential","breach","baa","data protection","safeguard"],
   block:"Member-level data handling occurs inside BAA-covered infrastructure with access controls and audit logging. Field tools are de-identified by design. [COMPANY FACT NEEDED: security certifications, hosting posture, incident-response policy — from IT/compliance; never claim a certification unverified.]"},
  {key:"pricing", label:"Pricing / cost proposal", keywords:["price","pricing","cost","rate","budget","fee","compensation","invoice","payment"],
   block:"Pricing anchors from published fee schedules: NC HOP $7.92/meal (2024 schedule), CalAIM $9.50/meal published midpoint ($7–$12 band), MassHealth $14.86 expected / $18.58 max. [COMPANY FACT NEEDED: this section is built WITH finance — pricing is a finance sign-off, never a sales draft.]"},
  {key:"eligibility", label:"Eligibility, enrollment & outreach", keywords:["eligibility","enrollment","outreach","referral","identification","engagement","recruit","member contact"],
   block:"Enrollment rides the dual-trigger documentation standard: nutrition-sensitive condition plus screened, Z-coded need — captured in one pass at referral. Member outreach flows through the plan and community trusted-messenger partners; every screening channel is documented and every positive screen routes to a documented intervention (closed loop)."},
  {key:"references", label:"References", keywords:["reference","referee","client contact","letters of"],
   block:"[COMPANY FACT NEEDED: three references with named contacts, obtained WITH PERMISSION before submission — never list a reference who has not agreed.]"},
  {key:"financials", label:"Financial statements / insurance / bonding", keywords:["financial statement","audited","insurance","bond","liability","solvency","certificate of insurance"],
   block:"[COMPANY FACT NEEDED: audited financials, certificates of insurance, bonding — from finance/legal. These are mandatory-form kills if missed; flag to the owner on Day 1.]"},
  {key:"subcontract", label:"Subcontracting / MWBE / local participation", keywords:["subcontract","mwbe","dbe","minority","small business","local participation","supplier diversity"],
   block:"[COMPANY FACT NEEDED: subcontracting plan and any diversity-participation commitments — from legal/ops. Where community-based partners deliver outreach, this section is an asset: name the model, not specific partners, until agreements exist.]"}
];
TOOL_DATA.rfpSampleText = `SYNTHETIC SAMPLE — State Medicaid Managed Care Organization RFP excerpt (for demonstration only)
Section 3. Scope of Work.
3.1 The Contractor shall provide medically tailored meals to eligible members with qualifying chronic conditions as identified by the MCO.
3.2 The Contractor must verify delivery capability for all parishes in the service area prior to the start date.
3.3 Vendor must describe its process for identifying and documenting member food-security needs, including the screening instrument used and how positive screens are addressed within 30 days.
3.4 Describe your organization's experience delivering nutrition services to Medicaid populations, including at least two examples of similar programs.
3.5 Provide a staffing plan identifying key personnel, including clinical oversight by a licensed dietitian.
3.6 The Contractor shall submit quarterly reports including meals delivered, member engagement rates, and outcomes data.
3.7 Explain how member data will be protected in accordance with HIPAA requirements.
Section 4. Proposal Requirements.
4.1 Proposals must include a completed Attachment A (Vendor Certification Form) and Attachment B (Pricing Schedule), signed by an authorized representative.
4.2 Proposals must not exceed 25 pages, excluding attachments.
4.3 Proposals are due no later than 3:00 PM Central Time on September 15, 2026.
4.4 Provide three client references including contact name, phone, and email.
Section 5. Evaluation Criteria.
5.1 Proposals will be scored as follows: Technical approach 30 points; Experience and qualifications 25 points; Staffing plan 15 points; Reporting and data capabilities 15 points; Price 15 points.
5.2 The MCO reserves the right to reject any proposal that fails to include all mandatory forms.`;
TOOL_DATA.sources.unshift(
